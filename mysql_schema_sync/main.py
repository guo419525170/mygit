...
if __name__ == '__main__':
    ...
    #删除源库中不存在而目标库中存在的表
    if del_table:
       drop_table_sql(tagcoursor,del_table)
    ...
    #需要比较表结构的表
        ...
        #source,target中都有，需要对比，并修改
            for column in same_column:
                ...
                targte_column_type=clumntype_get(tagcoursor,tab,column,db_dest)
                front_column = front_clumn_get(srccoursor, tab, db_source, column)
                #print(front_column)
            # 如果两个列表完全一致，则不需要修改
        ...
        # source中没有，target中有，需要删除
            if del_column:
                for column in del_column:
                    front_column = front_clumn_get(srccoursor, tab, db_source, column)
                    column_type = clumntype_get(tagcoursor, tab, column,db_dest)
                    sql = column_sql(tab, column, column_type, 'd',front_column)
                    f.write(sql+'\n')
        #源表中又，目标表中没有的索引需要添加
            if add_key:
                for key in add_key:
