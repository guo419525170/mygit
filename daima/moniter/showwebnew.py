...
def executesql(sql,area,types):
	if area == 'hengyang':
		port,host = 22,conf.hengyanghost
	if area == 'leiyang':
		port,host = 22,conf.leiyanghost
	if area == 'shaoyang':
		port,host = 22,conf.shaoyanghost
	if area == 'yueyang':
		port,host = 22,conf.yueyanghost
	if area == 'yongzhou':
		port,host = 22,conf.yongzhouhost
	if area == 'xiangxiang':
		port,host = 22,conf.xiangxianghost
	if area == 'haian':
...
class count:
	def GET(self,area,time):
		if time == 'day':
			sql='select count(1),hostname,error_log from moniter.error_info where datetime >= curdate() group by error_log,hostname order by 1 desc limit 15;'
		if time == 'week':
			sql='select count(1),hostname,error_log from moniter.error_info where datetime >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) group by error_log,hostname order by 1 desc limit 20;'
		if time == 'month':
			sql='select count(1),hostname,error_log from moniter.error_info where datetime >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) group by error_log,hostname order by 1 desc limit 30;'
...
class insertexcludelog:
	def GET(self,info,area):
		...
		arealist=['hengyang','leiyang','xiangxiang','yueyang','shaoyang','yongzhou','js3mj','nantong','xuzhou','haian','huaian','jinzhong']
		jsonload = []
		content = {}
